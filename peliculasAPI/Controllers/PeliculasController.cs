...
namespace peliculasAPI.Controllers
{
	...
	public class PeliculasController : ControllerBase
	{
		...
		public readonly string contenedor = "pelicula";

		public PeliculasController(AplicationDbContex context,
			IMapper mapper,
		...
		public async Task<ActionResult<PeliculaDTO>> Get(int id)
		{
			...
			if(await context.Ratings.AnyAsync(x=>x.PeliculaId == id))
			{
				promedioVoto = await context.Ratings
					.Where(x => x.PeliculaId == id).AverageAsync(x => x.Puntuacion);
				...
				if (HttpContext.User.Identity.IsAuthenticated)
				{
					...
					if(ratingDB != null) { votoUsuario = ratingDB.Puntuacion; }
				}
			}
		...
		public async Task<ActionResult<List<PeliculaDTO>>> Filtrar([FromQuery] PeliculaFiltrarDTO peliculasFiltrarDTO)
		{
			...
			await HttpContext.InsertaParametrosPaginacionEnCabecera(peliculasQueryable);
		...
		public async Task<ActionResult> Post([FromForm] PeliculaCreacionDTO peliculaCreacionDTO)
		{
			...
			{
				pelicula.Poster = await almacenadorArchivo.GuardarArchivo(contenedor, peliculaCreacionDTO.Poster);
			}
		...
		{
			var cines = await context.Cine.ToListAsync();
		...
		public async Task<ActionResult<PeliculasPutGetDTO>> PutGet(int id)
		{
			...
			var cinesNoSeleccionados = await context.Cine
		...
		public async Task<ActionResult> Put(int id, [FromForm] PeliculaCreacionDTO peliculaCreacionDTO)
		{
			...
			if(peliculaCreacionDTO.Poster != null)
			{
				pelicula.Poster = await almacenadorArchivo.EditarArchivo(contenedor, peliculaCreacionDTO.Poster, pelicula.Poster);
			}
		...
		public async Task<ActionResult> Delete(int id)
		{
			...
			await context.SaveChangesAsync();
			await almacenadorArchivo.BorrarArchivo(pelicula.Poster,contenedor);
			return NoContent();
		}
	}
}
